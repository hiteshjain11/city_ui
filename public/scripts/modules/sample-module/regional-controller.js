define(['angular', './sample-module'], function(angular, controllers) {
    ...
    controllers.controller('RegionalpageCtrl', ['$scope', '$log', 'PredixAssetService', 'PredixViewService', '$http', '$interval', '$compile', '$window', 'dataFactory', '$filter', 'TruckService', 'MapFilterService', function($scope, $log, PredixAssetService, PredixViewService, $http, $interval, $compile, $window, dataFactory, $filter, TruckService, MapFilterService) {
        ...
        dataFactory.getMapData()
            .then(function(response) {
                ...
                if(MapFilterService.filtertype == 'Garbage'){
                    $scope.mapDataTypes = [{ text: 'Hospital', selected: false }, { text: 'Truck', selected: false }, { text: 'Tank', selected: false }, { text: 'Garbage', selected: true }];
                    $scope.updateMapData();
                    setTimeout(function() {
    angular.element(".panelbg_0,.panelbg_1,.panelbg_2").removeClass("map-selected");
}, 300);

                }else if(MapFilterService.filtertype == 'Tank'){
                    $scope.mapDataTypes = [{ text: 'Hospital', selected: false }, { text: 'Truck', selected: false }, { text: 'Tank', selected: true }, { text: 'Garbage', selected: false }];
                    $scope.updateMapData();
                    setTimeout(function() {
    angular.element(".panelbg_0,.panelbg_1,.panelbg_3").removeClass("map-selected");
}, 300);
                }else{
                    $scope.mapDataTypes = [{ text: 'Hospital', selected: true }, { text: 'Truck', selected: true }, { text: 'Tank', selected: true }, { text: 'Garbage', selected: true }];
                    onloadMarkers();
                }
        ...
        $scope.updateMapData = function() {
                ...
                for (var kk = 0; kk < testDK.length; kk++) {
                    ffdata[kk] = $filter('filter')($scope.mapData, {
                        type: testDK[kk]
                    });
                }
